 ...
 *
 * The login form defaults to an Email tab (email + password + "Sign in").
 * Phone OTP lives behind the "Phone" tab and must be opened first.
 */

async function openPhoneTab(page: Page) {
...
test.describe('Login page', () => {
  ...
  test('renders login form with Google button and Email/Phone tabs', async ({ page }) => {
    ...
    // Email tab is the default — email + password inputs and Sign in button
  ...
  test('shows OTP input after sending OTP to a phone', async ({ page }) => {
    ...
    const errorDiv = page.locator('[class*="f87171"]').first()
    await expect(otpInput.or(errorDiv).first()).toBeVisible({ timeout: 8000 })
  })
  ...
  test('shows back button to change number after OTP step (if OTP provider configured)', async ({ page }) => {
    ...
    const otpVisible = await page.getByPlaceholder('000000').isVisible({ timeout: 7000 }).catch(() => false)
    if (otpVisible) {
  ...
  test('already authenticated user redirected to dashboard', async ({ page }) => {
