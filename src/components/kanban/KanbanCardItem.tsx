...
export function KanbanCardItem({ card, currentColumn, allColumns, onMove }: Props) {
  ...
  return (
    <>
      <div
        className={`bg-[#07070a] border border-[#1e1e2e] rounded-xl p-3.5 transition-all duration-200 hover:border-[#6366f1]/20 group cursor-pointer ${moving ? 'opacity-40 pointer-events-none' : ''}`}
        draggable
      ...
      {showDetail && (
        <KanbanCardModal card={card} onClose={() => setShowDetail(false)} />
      )}
    </>
  )
}
